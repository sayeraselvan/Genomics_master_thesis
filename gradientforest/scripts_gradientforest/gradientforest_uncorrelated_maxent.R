snp <- read.table("/netscratch/dep_coupland/grp_fulgione/siva/gradientforest_analyses/snpdata_gradientforest/snp.forR", header = T, row.names = 1)

library(raster)
library(rgdal)
clim.list <- dir("/netscratch/dep_coupland/grp_fulgione/siva/gradientforest_analyses/climatedata_gradientforest/Climate_uncor/", full.names=T, pattern='.tif')  #makes list of file paths for each layer
...
#write.table(clim.points, "clim.points_uncor_no_pcmn", sep="\t", quote=F, row.names=F)  #save the table for later use
clim.points
...
#env.gf <- clim.points[,c("bio_8", "bio_2", "bio_7", "bio_12", "bio_1", "bio_9", "bio_19", "bio_18", "bio_15", "bio_3", "ai_Apr")]
env.gf <- cbind(clim.points[,c("bio_8", "bio_2", "bio_7", "bio_12", "bio_1", "bio_9", "bio_19", "bio_18", "bio_15", "bio_3", "ai_Apr")], pcnm.keep)
...
saveRDS(gf, file="/netscratch/dep_coupland/grp_fulgione/siva/gradientforest_analyses/results_gradientforest/GF_uncorr.RDS")

pdf("/netscratch/dep_coupland/grp_fulgione/siva/gradientforest_analyses/results_gradientforest/results_plots/GF_VariableImportance_uncor.pdf")
plot(gf, plot.type = "O")
dev.off()

by.importance <- names(importance(gf))

pdf("/netscratch/dep_coupland/grp_fulgione/siva/gradientforest_analyses/results_gradientforest/results_plots/GF_TurnoverFunctions_uncor.pdf")
