...
setwd(paste0("/netscratch/dep_coupland/grp_fulgione/siva/gemma/RESULTS_GEA/", analysis_type, "/unpruned/no_covariates/without_FDR_correction_without_covariate_gemma_lmm/association_files_results_gemma"))
...
for (i in 1:length(f_short)) {
  ...
  # Define the FDR threshold function

  BC <- -log10(0.05 / nrow(gwscan))
  # Convert table to rehh format
  gwscan <- transform(gwscan, p_wald = -log10(p_wald))
  gwscan_rehh <- cbind.data.frame(gwscan$chr, gwscan$ps, gwscan$p_wald)
